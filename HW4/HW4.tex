...
\begin{enumerate}[label=(\alph*)]
  ...
  \end{enumerate}
  \item \textbf{Closed-form ridge solution via normal equations.}
    To find the minimizer $w_\lambda^\star$ of $L_\lambda(w)$, we set the gradient to zero:
    \[
      \nabla L_\lambda(w_\lambda^\star) = 0 \implies 2A^\top(Aw_\lambda^\star - y) + 2\lambda w_\lambda^\star = 0.
    \]
    Rearranging gives:
    \[
      (A^\top A + \lambda I_d) w_\lambda^\star = A^\top y.
    \]
    Since $A^\top A + \lambda I_d$ is invertible for $\lambda > 0$, we can solve for $w_\lambda^\star$:
    \[
      w_\lambda^\star = (A^\top A + \lambda I_d)^{-1} A^\top y.
    \]
  \item \textbf{SVD and spectrum of the Hessian.}
  \begin{enumerate}
    \item
      ...
      \]
    \item
      The eigenvalues of $H_\lambda$ can be found by considering the action of $H_\lambda$ on the basis vectors $v_i$:
      \[
  ...
  \begin{enumerate}
    \item
      Using the SVD of $A$, we can express $w_\lambda^\star$ as:
      \[
        w_\lambda^\star = V(\Sigma^2 + \lambda I_r)^{-1} \Sigma U^\top y.
      \]
      Expanding this, we get:
      \[
        w_\lambda^\star = \sum_{i=1}^r \frac{\sigma_i}{\sigma_i^2 + \lambda} (u_i^\top y) v_i,
      \]
    ...
    \item
      ...
      \[
        \alpha_i(\lambda) \to \frac{1}{\sigma_i} (u_i^\top y),
      \]
      which corresponds to the coefficients of the minimum-norm least squares solution:
      \[
        w_0^\star = A^+ y = \sum_{i=1}^r \frac{1}{\sigma_i} (u_i^\top y) v_i.
      \]
  \end{enumerate}
\end{enumerate}
\end{enumerate}

\includepdf[pages=-]{HW4_collab.pdf}
